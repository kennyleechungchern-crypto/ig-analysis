...
## v2（2026-09-16）：45 秒版 + CTA

每篇口播稿文件末尾追加了「## 45 秒版（tag: 导流 / 互动）」，长版保留。规则在 `prompts/kenny_voice.md` 的 v2 段：
四拍 130–180 字，CTA 放在 32 秒左右，最后一句永远是金句。

- **导流**（37 篇，`data/cta_tags.json`）：主题接得到「时间 / 忙 / 工作」的，CTA 固定机制 —— 点名「做业务的」→
  「22 号说明会」（4U 超级销冠系统，AI × 人性 极速成交力）→ 留言「时间」发链接。日期换了改稿里的「22 号」即可。
- **互动**（19 篇）：关系、情绪、看人类主题，CTA 用二选一留数字或一个词，不用开放问题。

`build_scripts_page.py` 生成的网页先显示 45 秒版，长版折叠，可按导流 / 互动筛选。
